"""
Integration tests for AI Agent Integration Service
Tests comprehensive AI agent functionality including service integrations
"""
...
from icpy.services.code_execution_service import Language, ExecutionStatus
from icpy.core.message_broker import MessageBroker, Message, MessageType


class TestAIAgentService:
    """Test AI Agent Integration Service functionality"""
    ...
    async def test_service_initialization(self, mock_message_broker):
        ...
        assert mock_message_broker.subscribe.call_count >= 4
    ...
    @pytest.mark.asyncio
    async def test_action_execution_context_operations(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_action_execution_cursor_position(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_action_execution_unregistered_agent(self, ai_agent_service):
    ...
    @pytest.mark.asyncio
    async def test_action_execution_with_service_mock(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_action_execution_error_handling(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_get_workspace_intelligence(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_execute_code_with_context(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_get_active_agents(self, ai_agent_service):
    ...
    @pytest.mark.asyncio
    async def test_get_action_history(self, ai_agent_service, sample_agent_id):
    ...
    async def test_global_service_instance(self):
        ...
        service1 = get_ai_agent_service()
        service2 = get_ai_agent_service()
        
        assert service1 is service2  # Same instance
        assert isinstance(service1, AIAgentService)
    ...
    async def test_service_lazy_initialization(self, ai_agent_service):
        ...
        # Mock the service imports to avoid actual initialization
        with patch('icpy.services.ai_agent_service.FileSystemService') as mock_fs:
            mock_fs.return_value = Mock()
            service = await ai_agent_service._get_filesystem_service()
            assert service is not None
            assert ai_agent_service._filesystem_service is not None

    @pytest.mark.asyncio
    async def test_unknown_action_type(self, ai_agent_service, sample_agent_id):
        ...
        action = AgentAction(
            ...
            action_type="UNKNOWN_ACTION",  # This will cause issues in _execute_action_impl
            parameters={}
        )
        
        # Test that the service handles unknown actions gracefully
        with pytest.raises(ValueError, match="Unknown action type"):
            await ai_agent_service._execute_action_impl(action)

    @pytest.mark.asyncio
    async def test_message_broker_integration(self, ai_agent_service, mock_message_broker):
        """Test message broker integration and event handling"""
        sample_agent_id = "test_agent"
        await ai_agent_service.register_agent(sample_agent_id)
        
        # Subscribe to events
        await ai_agent_service.subscribe_to_events(sample_agent_id, ["fs.file_created"])
        
        # Create a mock message
        mock_message = Mock()
        mock_message.type = "fs.file_created"
        mock_message.data = {"path": "/test/newfile.txt"}
        
        # Handle the event
        await ai_agent_service._handle_filesystem_event(mock_message)
        
        # Verify message broker was used
        assert mock_message_broker.publish.called

    @pytest.mark.asyncio
    async def test_agent_activity_tracking(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_action_history_storage(self, ai_agent_service, sample_agent_id):
    ...
    @pytest.mark.asyncio
    async def test_service_error_handling(self, ai_agent_service, sample_agent_id):
        ...
        # Patch the service initialization to fail
        with patch.object(ai_agent_service, '_get_filesystem_service', return_value=None):
            action = await ai_agent_service.execute_action(
                sample_agent_id,
                AgentActionType.READ_FILE,
                {"path": "/test/file.txt"}
