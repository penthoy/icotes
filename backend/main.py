...
# ========================================
# CRITICAL: These endpoints are TEMPORARY WORKAROUNDS added due to Pydantic incompatibility
# preventing ICPY modules from loading. They violate the ICPY architecture and MUST be removed
# once the Pydantic version issue is resolved.
#
# PROPER LOCATION: These endpoints should be served by:
# - backend/icpy/api/rest_api.py (lines 386-465) - REST API implementation
# - backend/icpy/services/filesystem_service.py - Actual file operations
# - Integration with message broker for real-time events
#
# ISSUES WITH CURRENT IMPLEMENTATION:
# 1. No integration with filesystem_service (proper async file ops)
# 2. No event broadcasting for real-time updates
# 3. No proper error handling/response format
# 4. No file watching or change detection
# 5. Direct file operations instead of service layer
# 6. Missing workspace path validation and security
#
# TODO: Remove this entire section (lines 521-647) after fixing Pydantic compatibility
...
@app.get("/api/files")
async def list_files(workspace_path: str = "/home/penthoy/ilaborcode/workspace"):
    """List files in workspace directory.
    
    TEMPORARY: Should use icpy.services.filesystem_service.list_directory()
    """
    try:
...
async def get_file_content(path: str):
    ...
    TEMPORARY: Should use icpy.services.filesystem_service.read_file()
    """
    try:
...
async def create_file(request: dict):
    ...
    TEMPORARY: Should use icpy.services.filesystem_service.write_file()
    with proper event broadcasting
    """
    try:
...
async def delete_file(path: str):
    ...
    TEMPORARY: Should use icpy.services.filesystem_service.delete_file()
    with proper event broadcasting
    """
    try:
        if not os.path.exists(path):
...
# END TEMPORARY FILE API ENDPOINTS
# ========================================
