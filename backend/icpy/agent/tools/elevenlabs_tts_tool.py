        ...
        try:
            ...
            if save_to_workspace:
                ...
                if save_result:
                    ...
                    if not ok:
                        logger.error(
                            "[ElevenLabsTTS] Save verification failed for %s | debug=%s",
                            absolute_path,
                            debug,
                        )
                        ...
                        response_data["save_error"] = "Audio generation succeeded but output file was not created (or was empty)"
                        response_data["save_debug"] = debug
                        return ToolResult(
                            success=False,
                            data=response_data,
                            error="Output file verification failed: audio file missing/empty after save",
                        )
                    ...
                    logger.info(f"[ElevenLabsTTS] Audio saved to: {absolute_path}")
                else:
                    response_data["saved"] = False
