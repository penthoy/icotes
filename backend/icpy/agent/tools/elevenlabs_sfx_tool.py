        ...
        try:
            ...
            if save_to_workspace:
                ...
                if save_result:
                    ...
                    if not ok:
                        logger.error(
                            "[ElevenLabsSFX] Save verification failed for %s | debug=%s",
                            absolute_path,
                            debug,
                        )
                        ...
                        response_data["save_error"] = "Sound effect generation succeeded but output file was not created (or was empty)"
                        response_data["save_debug"] = debug
                        return ToolResult(
                            success=False,
                            data=response_data,
                            error="Output file verification failed: sound file missing/empty after save",
                        )
                    ...
                    logger.info(f"[ElevenLabsSFX] Sound effect saved to: {absolute_path}")
                else:
                    response_data["saved"] = False
