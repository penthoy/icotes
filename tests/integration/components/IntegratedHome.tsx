...
const TerminalList: React.FC = () => {
  ...
  return (
    <div className="p-4">
      ...
      {terminals.length === 0 ? (
        <p className="text-gray-500">No terminals created</p>
      ) : (
        <ul className="space-y-1">
          {terminals.map((terminal) => (
            <li key={terminal.id} className="flex items-center space-x-2">
              <span className="text-sm font-mono">{terminal.name || terminal.id}</span>
