...
2 with modular design that can later be extended to do simple audio/video edits such as in audacity with some simple cuting and triming, we will not be doing this now, but this is to give a context on the kind of design we should make at this point and to choose the correct library for the job.

3 based on these context, your current task is to do research and choose the correct frame work for the job and to create the first version of the media player and create phased plan mediaplayer_plan.md under docs/plans feel free to search online if you require research, in order to make the best decision on the right library to use based on this repo with the context you're given, when done, please don't touch any code wait for my review on the plan.
