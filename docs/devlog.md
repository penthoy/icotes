...
## 2.4.5
--2.4.5:
- [x] Put Chat History to the center
- [x] fully test and record update video: ask for a report from last vid update to current.
- [x] Record video and squash all the bugs during recording.
- [x] Bugfix: video player seek isn't working
- [x] make chat window default bigger
- [x] Fix imagen aspect ratio bug: Upgraded from google-generativeai SDK (v0.8.5) to google-genai SDK (v1.62.0) which natively supports aspect_ratio parameter. The old SDK generated 1:1 images then resized them with PIL, causing quality loss. The new SDK uses `types.ImageConfig(aspect_ratio="16:9")` to generate at the correct ratio directly. Fixed by: 1) Upgraded google-genai>=1.60.0, fastapi>=0.115.0, httpx>=0.28.1, anyio>=4.8.0 in pyproject.toml. 2) Rewrote `_attempt()` to use new `client.models.generate_content()` API with `types.GenerateContentConfig(image_config=...)`. 3) Updated `execute()` to detect native aspect ratios and skip PIL resize. 4) Updated tests to mock new SDK structure. Supported ratios: 1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9. Changes: [backend/icpy/agent/tools/imagen_tool.py](backend/icpy/agent/tools/imagen_tool.py), [backend/pyproject.toml](backend/pyproject.toml), [backend/tests/icpy/agent/tools/test_imagen_tool.py](backend/tests/icpy/agent/tools/test_imagen_tool.py).
- [x] Fix write_doc PDF creation bug: PDFs were not being created on disk despite the tool claiming success. Root cause was that `FileSystemService` lacked a `write_file_binary` method - it was falling through to `write_file` which is for text, not binary data. Fixed by: 1) Adding `write_file_binary` method to `FileSystemService` with proper binary mode (`'wb'`) and verification. 2) Updated `write_doc_tool._write_file_bytes` to return dict with success/error instead of bool. 3) Added `_verify_file_exists` method to confirm file exists on disk before returning success. 4) Updated tests to match new return format. Changes: [backend/icpy/services/filesystem_service.py](backend/icpy/services/filesystem_service.py), [backend/icpy/agent/tools/write_doc_tool.py](backend/icpy/agent/tools/write_doc_tool.py), [backend/tests/icpy/agent/tools/test_write_doc_tool.py](backend/tests/icpy/agent/tools/test_write_doc_tool.py).
- [x] Fix generic widget loading icon: Removed all "Processing tools..." and "Executing tools..." text from loading indicators. Both the generic ToolCallWidget and ProgressWidget now show only a clean spinning loader icon without any text when tools are running. The loading indicator appears within each widget itself. Changes made to [src/icui/components/chat/ChatMessage.tsx](src/icui/components/chat/ChatMessage.tsx), [src/icui/components/chat/ToolCallWidget.tsx](src/icui/components/chat/ToolCallWidget.tsx), and [src/icui/components/chat/widgets/ProgressWidget.tsx](src/icui/components/chat/widgets/ProgressWidget.tsx).
- [x] Add missing API key fields into the settings page: Added ELEVENLABS_API_KEY and ATLASCLOUD_API_KEY to ApiKeyModal.tsx Services tab. Also updated .env.example to include all API keys used in the codebase (DEEPSEEK, DASHSCOPE, MOONSHOT, OLLAMA_URL for AI models, and ELEVENLABS, ATLASCLOUD for services).
- [x] Fix widget duration timer (BLOCKED - needs investigation): Duration shown immediately after tool completion doesn't match duration after page refresh. Live timer updates work correctly, but final duration calculation is inconsistent. Multiple approaches attempted including reply_to timestamps, backend timestamp consistency, and useMemo dependencies. Issue documented in [docs/tickets/tool_duration_bug_ticket.md](docs/tickets/tool_duration_bug_ticket.md) for further investigation. Partial improvements made to timing infrastructure that should be preserved.

## 2.4.2
- [x] Update version
- [x] Update Kimi k2.5
- [x] Change default agent
- [x] Build Docker
- [x] Add https://www.atlascloud.ai/models/list support

- [x] Migrate frontend tooling to bun: Switched scripts/setup/start-dev/Dockerfile to bun, added bun lockfile, updated docs, and made preview service prefer bun. Files: package.json, setup.sh, start-dev.sh, Dockerfile, backend/icpy/services/preview_service.py, docs/SETUP.md, ENVIRONMENT_SETUP.md, .github/copilot-instructions.md
- [x] Fix PDF reader divider drag: Divider resizing could feel sticky/slow when a PDF (iframe-based native viewer) was open because the iframe captured mouse events. Added a global resize-drag mode that disables iframe/embed/object pointer events while dragging split dividers. Files: [src/icui/components/ICUISplitPanel.tsx](src/icui/components/ICUISplitPanel.tsx), [src/icui/styles/icui.css](src/icui/styles/icui.css)
- [x] Fix Gemini image preview in Image Generation widget: Gemini native tool streaming previously truncated JSON tool results (breaking widget parsing and previews). Now emits valid JSON and sends a compact `generate_image` payload (keeps `imageUrl`/`fullImageUrl`, avoids base64 thumbnails). Added tests. Files: [backend/icpy/agent/core/llm/gemini_native_client.py](backend/icpy/agent/core/llm/gemini_native_client.py), [backend/tests/icpy/agent/llm/test_gemini_native_tool_formatting.py](backend/tests/icpy/agent/llm/test_gemini_native_tool_formatting.py)
- [x] ElevenLabs Sound Effects Tool: Created `text_to_sound_effects` agent tool using ElevenLabs Sound Effects API. Generates realistic and fantastical sound effects from text descriptions with configurable duration (0.5-30s, default 1s), prompt influence (0-1), optional seamless looping, and hop-aware file saving to workspace/sounds/. Supports MP3 output. Added conservative 1-second default to minimize credit usage during testing. Added comprehensive tests (23 tests passing). Files: [backend/icpy/agent/tools/elevenlabs_sfx_tool.py](backend/icpy/agent/tools/elevenlabs_sfx_tool.py), [backend/tests/icpy/agent/tools/test_elevenlabs_sfx_tool.py](backend/tests/icpy/agent/tools/test_elevenlabs_sfx_tool.py), [backend/icpy/agent/tools/__init__.py](backend/icpy/agent/tools/__init__.py)
- [x] ElevenLabs Music Tool: Created `text_to_music` agent tool using ElevenLabs Music API (paid users only). Generates custom music from text prompts describing style, tempo, instruments, and mood. Supports configurable duration (0.5-120s, default 5s), prompt influence control (0-1, default 0.3), and hop-aware file saving to workspace/sounds/. Added conservative 5-second default to minimize credit usage during testing. Added comprehensive tests (23 tests passing). Files: [backend/icpy/agent/tools/elevenlabs_music_tool.py](backend/icpy/agent/tools/elevenlabs_music_tool.py), [backend/tests/icpy/agent/tools/test_elevenlabs_music_tool.py](backend/tests/icpy/agent/tools/test_elevenlabs_music_tool.py), [backend/icpy/agent/tools/__init__.py](backend/icpy/agent/tools/__init__.py)
- [x] ElevenLabs Speech-to-Text Tool: Created `speech_to_text` agent tool using ElevenLabs Scribe API. Transcribes audio files (mp3, wav, m4a, ogg, flac, aac, wma, webm) to text with 99 language support (auto-detection or explicit), speaker diarization, audio event tagging (laughter, applause, music), word/segment timestamps, and hop-aware file reading. Fixed tool description to clarify that language_code should be omitted entirely (not empty string) for auto-detection. Added comprehensive tests (28 tests passing). Files: [backend/icpy/agent/tools/elevenlabs_stt_tool.py](backend/icpy/agent/tools/elevenlabs_stt_tool.py), [backend/tests/icpy/agent/tools/test_elevenlabs_stt_tool.py](backend/tests/icpy/agent/tools/test_elevenlabs_stt_tool.py), [backend/icpy/agent/tools/__init__.py](backend/icpy/agent/tools/__init__.py)
- [x] ElevenLabs Text-to-Speech Tool: Created `text_to_speech` agent tool using ElevenLabs API. Supports multiple voices (george, rachel, bella, etc.), multilingual models (eleven_multilingual_v2), various output formats (mp3, wav, ulaw), voice settings (stability, similarity_boost, style), and hop-aware file saving to workspace/sounds/. Enhanced to automatically fetch and resolve custom voice names from user's ElevenLabs account (e.g., 'tao2') via API. Added comprehensive tests (34 tests passing). Files: [backend/icpy/agent/tools/elevenlabs_tts_tool.py](backend/icpy/agent/tools/elevenlabs_tts_tool.py), [backend/tests/icpy/agent/tools/test_elevenlabs_tts_tool.py](backend/tests/icpy/agent/tools/test_elevenlabs_tts_tool.py), [backend/icpy/agent/tools/__init__.py](backend/icpy/agent/tools/__init__.py), [backend/pyproject.toml](backend/pyproject.toml)
- [x] File Type Detection Refactor: Refactored `languageDetection.ts` to `fileTypeDetection.ts` to support broader file type handling beyond just programming languages. Updated all references including function names (`detectFileTypeFromExtension`, `supportedFileTypes`, etc.). Added backward compatibility aliases for smooth transition. Prepares codebase for multimedia and multimodal file support in the editor. Files: [src/icui/components/editor/utils/fileTypeDetection.ts](src/icui/components/editor/utils/fileTypeDetection.ts), [src/icui/components/editor/index.ts](src/icui/components/editor/index.ts), [src/icui/components/panels/ICUIEditor.tsx](src/icui/components/panels/ICUIEditor.tsx), [src/icui/components/editor/components/LanguageSelectorModal.tsx](src/icui/components/editor/components/LanguageSelectorModal.tsx)
- [x] PDF Editor Support: Added PDF viewing capability to the editor. PDF files can now be opened and viewed using the browser's native PDF viewer. Follows the same pattern as image support with automatic file type detection. Files: [src/icui/components/editor/components/PDFViewerPanel.tsx](src/icui/components/editor/components/PDFViewerPanel.tsx), [src/icui/components/editor/utils/fileTypeDetection.ts](src/icui/components/editor/utils/fileTypeDetection.ts), [src/icui/components/panels/ICUIEditor.tsx](src/icui/components/panels/ICUIEditor.tsx), [src/icui/components/editor/index.ts](src/icui/components/editor/index.ts)
- [x] Image Editor Zoom & Pan: Added bottom toolbar with zoom/pan tool buttons, zoom controls (in/out/fit/100%), mouse wheel zoom, click-drag panning, and design structure for future tools (crop, annotate). File: [src/icui/components/editor/components/ImageViewerPanel.tsx](src/icui/components/editor/components/ImageViewerPanel.tsx)
- [x] Document Processor Tools (read_doc/write_doc): Added agent tools for reading and writing office documents (Excel, Word, PDF, PowerPoint, CSV). Created modular doc_processor package with format handlers, comprehensive tests. Files: [backend/icpy/agent/tools/read_doc_tool.py](backend/icpy/agent/tools/read_doc_tool.py), [backend/icpy/agent/tools/write_doc_tool.py](backend/icpy/agent/tools/write_doc_tool.py), [backend/icpy/agent/tools/doc_processor/](backend/icpy/agent/tools/doc_processor/), [docs/plans/docprocessor_plan.md](docs/plans/docprocessor_plan.md)
- [x] Document Truncation for LLM Context: Added smart content truncation to `read_doc` tool to prevent LLM context overflow. Features include: auto-truncation at ~2K tokens (8K chars), line range extraction (start_line/end_line like head/tail), page range for PDFs, summary_only mode, and pagination hints for agents to read more. Files: [backend/icpy/agent/tools/read_doc_tool.py](backend/icpy/agent/tools/read_doc_tool.py), [backend/tests/icpy/agent/tools/test_read_doc_tool.py](backend/tests/icpy/agent/tools/test_read_doc_tool.py), [docs/plans/docprocessor_plan.md](docs/plans/docprocessor_plan.md)
- [x] Fix uvicorn reload breaking WebSockets: Agent `pip install` commands caused uvicorn to reload (watching .venv changes), disconnecting all WebSocket clients. Added `--reload-exclude '.venv/*'` to uvicorn commands. Files: [package.json](package.json), [start-dev.sh](start-dev.sh)
- [x] Add Media player that can play audio and video, similar to how vlc can play, but focus on the most used file formats such as mp3, wav, mp4, mov formats, so that when clicked on one of these media files, they can be previewed and played.

1 make sure to create this with a modular design so that it doesn't just play on the Editor, it can also be used as a widget later, when we can use text to speech AI to generate audio or text to video to play video later.

2 with modular design that can later be extended to do simple audio/video edits such as in audacity with some simple cutting and trimming, we will not be doing this now, but this is to give a context on the kind of design we should make at this point and to choose the correct library for the job.

3 based on these context, your current task is to do research and choose the correct framework for the job and to create the first version of the media player and create phased plan mediaplayer_plan.md under docs/plans feel free to search online if you require research, in order to make the best decision on the right library to use based on this repo with the context you're given, when done, please don't touch any code wait for my review on the plan.
- [x] Add audio player and simple video player
- [x] bug groq oss 120 doesn't work
- [x] Atlascloud Video Tool Phase 1: Core Infrastructure - Created atlascloud client library with async HTTP client, Pydantic models for requests/responses, comprehensive error handling, and polling mechanism for video generation. Added `get_atlascloud_client()` to clients.py for LLM inference support. Files: [backend/icpy/agent/tools/atlascloud/](backend/icpy/agent/tools/atlascloud/), [backend/icpy/agent/clients.py](backend/icpy/agent/clients.py), [docs/plans/videotool_plan.md](docs/plans/videotool_plan.md), [backend/tests/icpy/agent/tools/test_atlascloud_client.py](backend/tests/icpy/agent/tools/test_atlascloud_client.py) - 25 unit tests passing
